...
The categories (\texttt{Theory-HEP}, \texttt{Phenomenology-HEP} and \texttt{Experiment-HEP}) that we trained our classifier to predict have been assigned in different ways in the Inspire HEP database. Although the bulk of them were categorized based on arXiv's classification system, some papers were not, especially those published before arXiv established its dominance in the 1990s. It might be unclear whether these classification procedures are consistent. Therefore, in figure \ref{fig:stability}, we show that the performance of the text-classifier is nonetheless roughly stable throughout the time-period considered (1980--2020). The various classification procedures must therefore be somewhat consistent, as the constant weights of our classifier are able to capture the underlying linguistic identity of each of the corresponding scientific cultures. 

\begin{figure}
    \centering
    \includegraphics{Fig8.eps}
    \caption{Accuracy of the text-classifier from Section \ref{section:method_subcultures} as a function of the papers' years of publication. The accuracy is roughly constant across time for each of the three categories. Variations }
    \label{fig:stability}
\end{figure}

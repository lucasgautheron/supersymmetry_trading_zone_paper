...
The magnitude of the observed variations suggests that the frequency of trades may have itself significantly varied. In order to verify this, we therefore compute the overall trade probability $P(\text{trade})_{i\to j}$ as elaborated in Section \ref{section:method_trading_zone}. The results are shown in Figure \ref{fig:cross_dynamics}. They demonstrate that phenomenology has become relatively much less reliant on theory since 2001, as $P(\text{trade})$ has decreased from 13\% down to 7\% --- in other words, the trading zone has shrinked. A significant part of the decrease seems to be concomitant with the start of the \gls{lhc} in 2010. Theory seems more self-reliant overall and rather unaffected by the effect of the \gls{lhc}. This suggests the \gls{lhc} has contributed to further decouple these two subcultures, even though ``theory'' was itself already rather autonomous prior to its start.
...
Figure \ref{fig:cross_dynamics} does not show whether trades involving older references are equally affected by the observed decoupling as trades drawing from more recent work. In order to verify this it is necessary to break down the probability of trade not just as a function of the year of publication of the papers citing other work, but also as a function of the year of publication of the cited papers. The results of such an analysis are available in Appendix \ref{appendix:trading_zone_years} for curious readers. They suggest that former theoretical work (e.g. from the early 2000s) remains somewhat commensurable with present phenomenological developments (late 2010s) contrary to more recent theoretical work.
...
\subsection{\label{appendix:trading_zone_years}Trading zone between two evolving cultures}

As one examines trades between two scientific cultures or disciplines, it is important to keep in mind that these are undergoing constant transformations. As a result, that a, say, phenomenological paper (published at a year $y_i$) cites a theoretical paper (published at a year $y_j\leq y_i$) does not establish a connection between these cultures in general but between these cultures as they existed at these specific times. It is therefore useful to examine the intensity of trades as a function of both the year of the citation and the year of publication of the cited paper. The results for trades between the theoretical and phenomenological subcultures are shown in Figure \ref{fig:trading_zone_years}. 

\begin{figure*}
    \centering
    \includegraphics{Fig10.eps}
    \caption{\textbf{Intensity of trades as a function of the year of publication of the cited paper (x-axis) and of the citing paper (y-axis).}
    The upper matrix shows the probability of trades that consist in citations of theoretical papers that originate from phenomenological paper. The lower matrix shows the probability of trades that consist in citations of phenomenological papers originating from theoretical papers. Black circles are proportional to the probability that citations at a given year (read vertically) refer to papers published at a given year (read horizontally).}
    \label{fig:trading_zone_years}
\end{figure*}

In the case of phenomenological papers drawing from theoretical papers, most of the variation in the  probability of trades seems to be accounted for by the year of publication of the cited work ($R^2=0.58$). This suggests that phenomenological developments have not kept pace with recent theoretical developments.

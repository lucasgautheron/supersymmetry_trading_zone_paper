...
%DIF DEL first_submission.tex   Thu Mar 30 12:42:36 2023
%DIF ADD main.tex               Thu Mar 30 12:41:11 2023
...
\gls{hep}}\hspace{0pt}%DIFAUXCMD
) and for the more general question of the plurality of science from a quantitative perspective}\DIFaddend .
...
. Instead}\DIFaddend , the latter were most often proponents of a \DIFdelbegin \DIFdel{methodological }\DIFdelend \DIFaddbegin \DIFadd{``methodological'' }\DIFaddend form of unity \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(Ibid., p.~233), }\DIFaddend according to which the field is bound together by \DIFdelbegin \DIFdel{a shared commitment to a common set of norms }\DIFdelend \DIFaddbegin \DIFadd{common norms and conceptual tools \mbox{%DIFAUXCMD
\citep[p.~267]{Cat1998}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend rather than by relations of logical deduction from the most fundamental to the least fundamental theories. \DIFaddbegin \DIFadd{Indeed, this view provided an intellectual and philosophical basis for elevating the prestige of condensed-matter physics \mbox{%DIFAUXCMD
\citep[p.~148--149]{martin2018solid}}\hspace{0pt}%DIFAUXCMD
, thus putting condensed matter physicists and high-energy physicists on a more equal footing.
}\DIFaddend 

Even within one subfield, such as particle physics, the nature of the relationship between the objects manipulated by, say, experimentalists (for instance, tracks within a cloud-chamber, or electric signals from a sensor) and the more abstract entities manipulated by theorists (e.g. ``quarks'', ``gluons'', ``strings'', etc.) \DIFdelbegin \DIFdel{remains unclear. }\DIFdelend \DIFaddbegin \DIFadd{remained the subject of much philosophical debate; positivists would grant experiment a more fundamental status, by defending its ability to provide robust empirical statements stable against theory change. Others, such as Kuhn, opposed that empirical statements cannot be singled out from a theoretical paradigm, and stressed the theory-ladeness of experimental reports \mbox{%DIFAUXCMD
...
\DIFadd{The authors acknowledge funding from XXX.
}
...
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Topics and their correlation with categories}}
\DIFaddend 

\DIFaddbegin \DIFadd{Below, we evaluate how topics compare with the classification of the literature. For that, we generated a 2D representation of the semantic space by applying t-SNE on the distance matrix $1-R_{ij}$ where $R_{ij}$ is the correlation matrix for the 75 topics of the topic model. The t-SNE transformation aims to preserve distances, such that highly correlated topics should appear close on the map. We then colored each topic according to the category (among theory, phenomenology and experiment) that had the highest average contribution for this topic. Topics related to supersymmetry are labeled.
}

\begin{figure}
\includegraphics[]{Fig8.eps}
\caption{\label{fig:tsne}\DIFaddFL{Semantic map extracted from the topic model using t-SNE transformation. Each dot represents a topic.}}
\end{figure}

\DIFadd{Although the t-SNE transformation does not yield very stable results, 
}

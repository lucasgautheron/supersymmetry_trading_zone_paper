...
%DIF DEL first_submission.tex   Wed Apr  5 18:15:33 2023
%DIF ADD main.tex               Wed Apr  5 18:14:14 2023
...
\usepackage[backend=biber, style=apa, sortcites=false, uniquelist=false,natbib=true,doi=false,isbn=false,url=false,eprint=false,pagetracker, ibidtracker=constrict]{biblatex} %DIF > 
\AtEveryBibitem{ %DIF > 
\clearfield{location} %DIF > 
\clearfield{entrysubtype} %DIF > 
} %DIF > 
...
\institute{
%DIF 75-76c80-81
%DIF <  Lucas Gautheron \\   \email{lucas.gautheron@gmail.com}\\\\
%DIF <  Elisa Omodei \\   \email{omodeie@ceu.edu}\\\\
%DIF -------
 Lucas Gautheron \\   \email{lucas.gautheron@gmail.com}\\ORCID: 0000-0002-3776-3373\\\\ %DIF > 
 Elisa Omodei \\   \email{omodeie@ceu.edu}\\ORCID: 0000-0002-6748-5124\\\\ %DIF > 
%DIF -------
 \affaddr{\affmark[1]Interdisciplinary Centre for Science and Technology Studies (IZWT), University of Wuppertal, Germany}\\
%DIF 78c83
%DIF <  \affaddr{\affmark[2]Université Paris-Cité, Paris, France}\\
%DIF -------
 \affaddr{\affmark[2]Sciences Po, médialab, Paris, France}\\ %DIF > 
%DIF -------
...
(\ref{section:galison}). We will then provide the necessary background knowledge for understanding the context of our case-study before laying out our hypotheses: i) that theory and phenomenology\DIFaddbegin \DIFadd{, over the historical period considered (1980--2020), }\DIFaddend are to be regarded as two distinct theoretical subcultures within high-energy physics\DIFdelbegin \DIFdel{, ii) between which }\DIFdelend \DIFaddbegin \DIFadd{; ii) that }\DIFaddend supersymmetry generated diverse research programs, \DIFdelbegin \DIFdel{iii) and which sustained successful trades }\DIFdelend \DIFaddbegin \DIFadd{some being phenomenological and some being more theoretical; and iii), that supersymmetry significantly contributed to sustain successful trades between theory and phenomenology }\DIFaddend until it was put in doubt by experimental data (\ref{section:hep_susy}). We \DIFdelbegin \DIFdel{will close our introduction by elaborating on our }\DIFdelend \DIFaddbegin \DIFadd{then elaborate our }\DIFaddend motivation for addressing these hypotheses through quantitative methods (\ref{section:quantitative})\DIFdelbegin \DIFdel{, at which point we will present the data, method, and results of our project. }\DIFdelend \DIFaddbegin \DIFadd{. Then, section \ref{section:method} details the quantitative methods that were deployed in order to address each of the three claims put forward in the introduction. It starts with a description of the data on which our analysis rests and how it was collected (\ref{section:data}). Subsection \ref{section:method_subcultures} elaborates quantitative methods for assessing the level of semantic and social autonomy of certain categories (subcultures), and applies these methods to the two theoretical subcultures in \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
. The next subsection (\ref{section:method_plurality}) elaborates a methodology based on topic models in order to address the ``plasticity'' and ``plurality'' of supersymmetry, which can in principle be applied to all ``boundary objects'', i.e. those objects that can be traded between distinct subcultures while preserving and sustaining their distinctness. Finally, subsection \ref{section:method_trading_zone} provides a quantitative model for locating ``trading zones'' or more broadly concepts that enhance trades between subcultures (or scientific disciplines in general), and applies the model to the exchanges between the theoretical subcultures of \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
. Section \ref{section:application} reveals and interprets the results of these analyses. Finally, Section \ref{section:discussion} explores the consequences of this work, both for our case study (supersymmetry within \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
) and for the more general question of the plurality of science from a quantitative perspective.
}\DIFaddend 

\DIFaddbegin \section{\label{section:background}\DIFadd{Background}}

\DIFaddend \subsection{Subcultures and trading zones: Galison's approach to the plurality of science}\label{section:galison}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The fact that science is highly diverse is a challenge to those who, for various reasons}\footnote{\DIFdel{See \mbox{%DIFAUXCMD
\citealt{Hacking1996,Bechtel2007} }\hspace{0pt}%DIFAUXCMD
for an overview of the various commitments underlying the Unity of Science ``ideology'' and of the attempts to unify the sciences.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{, have sought to defend its unity. }\DIFdelend \DIFaddbegin 

%DIF >  The fact that science is highly diverse is a challenge to those who, for various reasons\footnote{See \citealt{Hacking1996,Bechtel2007} for an overview of the various commitments underlying the Unity of Science ``ideology'' and of the attempts to unify the sciences.}, have sought to defend its unity.
\DIFaddend If science is \DIFdelbegin \DIFdel{indeed }\DIFdelend a unified enterprise, what is the nature of the relationship between fields as diverse as physics, \DIFdelbegin \DIFdel{chemistry, }\DIFdelend biology, psychology, or economics? Can we translate all the concepts of these disciplines into a basic (say, physical) scientific language, as Carnap proposed? Or, are all these fields so incommensurable and autonomous that it is impossible to translate their respective entities, laws, and explanations from one's language to another's, as proponents of a pluralistic view defend (e.g. \citet{Suppes1978,Dupre1983,Cartwright1999})? Disciplines themselves can be so diverse, too, that the nature of what makes their own unity is not necessarily obvious. For instance, the nature of the unity of physics has been the matter of much debate, with sometimes serious political implications: reductionist views (which imply that high-energy physics \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend the most fundamental, since \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend supposedly entail any higher-level theory) were mobilized to justify the funding of large particle physics facilities \citep{Cat1998}, potentially to the detriment of more ``useful'' projects, as certain \DIFdelbegin \DIFdel{condensed-matter physicists argued \mbox{%DIFAUXCMD
\citep{martin2018solid}}\hspace{0pt}%DIFAUXCMD
. As a result}\DIFdelend \DIFaddbegin \DIFadd{condensed matter physicists argued \mbox{%DIFAUXCMD
\citep[Ch.~9]{martin2018solid}}\hspace{0pt}%DIFAUXCMD
. Instead}\DIFaddend , the latter \DIFaddbegin \DIFadd{argued that macroscopic systems have emergent properties that cannot be derived from ``fundamental'' laws. They }\DIFaddend were most often proponents of a \DIFdelbegin \DIFdel{methodological }\DIFdelend \DIFaddbegin \DIFadd{``methodological'' }\DIFaddend form of unity \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(Ibid., p.~233), }\DIFaddend according to which the field is bound together by \DIFdelbegin \DIFdel{a shared commitment to a common set of norms }\DIFdelend \DIFaddbegin \DIFadd{shared norms and conceptual tools \mbox{%DIFAUXCMD
\citep[p.~267]{Cat1998}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend rather than by relations of logical deduction from the most fundamental to the least fundamental theories. \DIFaddbegin \DIFadd{This view provided an intellectual and philosophical basis for elevating the prestige of condensed matter physics \mbox{%DIFAUXCMD
\citep[p.~148--149]{martin2018solid}}\hspace{0pt}%DIFAUXCMD
, thus putting condensed matter and high-energy physicists on a more equal footing.
}\DIFaddend 

Even within \DIFdelbegin \DIFdel{one subfield , such as }\DIFdelend \DIFaddbegin \DIFadd{the subfield of }\DIFaddend particle physics, \DIFaddbegin \DIFadd{there is a strong contrast between theorists and experimentalists. In fact, }\DIFaddend the nature of the relationship between the objects manipulated by, say, experimentalists (for instance, tracks within a cloud-chamber, or electric signals from a sensor) and the more abstract entities manipulated by theorists (e.g. ``quarks'', ``gluons'', ``strings'', etc.) \DIFdelbegin \DIFdel{remains unclear. }\DIFdelend \DIFaddbegin \DIFadd{has been the subject of much philosophical debate. Inheriting a positivist view, some would grant experiment a more fundamental status, by defending its ability to provide robust empirical statements that could dictate theoretical  change. Others, such as Kuhn, argued that empirical statements cannot be isolated from a theoretical paradigm and emphasized the``primacy'' of theory \mbox{%DIFAUXCMD
\citep{Galison1988}}\hspace{0pt}%DIFAUXCMD
}\footnote{\DIFadd{For instance, in his historical account of the discovery of quarks, \mbox{%DIFAUXCMD
\citet[p.~411]{pickering1984constructing} }\hspace{0pt}%DIFAUXCMD
endorses the Kuhnian view: ``To attempt to choose between old- and new-physics }[\DIFadd{gauge}] \DIFadd{theories on the basis of a common set of phenomena }[\DIFadd{experimental facts}] \DIFadd{was impossible: the theories were integral parts of different worlds, and they were incommensurable''. Instead, Galison emphasizes the relative continuity and robustness of experimental ``facts'', across theoretical changes.}}\DIFadd{. }\DIFaddend It is in order to \DIFdelbegin \DIFdel{address this challenge that Galison 's }\DIFdelend \DIFaddbegin \DIFadd{overcome this debate about the relationship between experiment and theory within the context of physics that Galison originally developed his }\DIFaddend concepts of subcultures and trading zones  \DIFdelbegin \DIFdel{were developed \mbox{%DIFAUXCMD
\citep{galison1987how,galison1997image}}\hspace{0pt}%DIFAUXCMD
, although }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{galison1987how,galison1997image}}\hspace{0pt}%DIFAUXCMD
. However, }\DIFaddend these notions may apply more generally whenever \DIFdelbegin \DIFdel{different }\DIFdelend \DIFaddbegin \DIFadd{distinct }\DIFaddend scientific communities attempt to overcome difficulties to communicate and achieve coordination \citep[p.~8]{Collins2010}. Consequently it is useful in a much broader range of contexts than the narrow case of physics; for instance, it is generally useful for studying the dynamics of interactions between disciplines in science\footnote{For example, \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Kemman2021} }\hspace{0pt}%DIFAUXCMD
describes }\DIFaddend Digital History \DIFdelbegin \DIFdel{can be regarded }\DIFdelend as a trading zone\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Kemman2021}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend .}. Below, we propose a brief summary of the concepts of subcultures and trading zones and the rationale for their introduction.

The notion of subcultures was introduced by \citet{galison1987how,Galison1988} in order to account for two characteristics of high-energy physics: first, that it is subject to a strong division of labor, such that ``theory'', ``experiment'' and ``instrumentation'' are carried out by different groups of people \citep[p.~138]{galison1987how}, with \DIFdelbegin \DIFdel{very distinct skill sets , manipulating quite distinct }\DIFdelend \DIFaddbegin \DIFadd{their own skill sets and }\DIFaddend bodies of knowledge; and second, that each of these ``subcultures'' are partially autonomous, i.e., none of them are completely subordinate to the others. We can highlight two tangible components of such subcultures: a social component--the community of practitioners--and a linguistic component--the language specific to each community.
...
\DIFdelbegin \DIFdel{Consequently }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, we hypothesized that }\DIFaddend supersymmetry should be losing its ability to sustain trades between theory and phenomenology. Therefore, we will evaluate the following claim:
...
More broadly, this \DIFdelbegin \DIFdel{works }\DIFdelend \DIFaddbegin \DIFadd{work }\DIFaddend will add to quantitative studies of science literature, by helping to fill a gap that has come to the attention of the community.  As stressed by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Kang2020,Leydesdorff2020,Bowker2020}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Leydesdorff2020,Kang2020,Bowker2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , quantitative and qualitative studies of science have mostly diverged in their goals and ``world views'', urging the need to ``bridge the gap'' between them. We propose, therefore, a bridge connecting these two forms of scientific study. First, we demonstrate that quantitative methods can address questions raised by the philosophy, history, and sociology of physics. Moreover, we show that concepts from qualitative 
 science studies can give structure to quantitative methods, in line with the call by \citealt{Heinze2020} to inform quantitative analyses with ``middle-range theories'' (of which Galison's trading zones are an example). As a result, our methods \DIFdelbegin \DIFdel{can be applied to much more diverse contexts than }\DIFdelend \DIFaddbegin \DIFadd{are in principle meaningful in any context where such a theory is valid -- whenever scientific cultures attempt to achieve coordination --, much beyond }\DIFaddend the case-study proposed in this paper.%For that, we deploy a range of methods informed by our conceptual framework, that can be applied to scientific literature at large.

\DIFdelbegin \DIFdel{The remainder of the paper is organized as follows. Section \ref{section:data} describes the data on which our analysis rests and how it was collected. Section \ref{section:method} details the quantitative methods that were deployed in order to address each of the three claims put forward in the introduction. The first subsection (\ref{section:method_subcultures}) elaborates quantitative methods for assessing the level of semantic and social autonomy of certain categories (subcultures), and applies these methods to the two theoretical subcultures in \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
. The second subsection (\ref{section:method_plurality}) elaborates a methodology based on topic models in order to address the ``plasticity'' and ``plurality'' of supersymmetry, which can in principle be applied to all ``boundary objects'', i.e. those objects that can be traded between distinct subcultures while preserving and sustaining their distinctness. The third subsection (\ref{section:method_trading_zone}) provides a quantitative model for locating ``trading zones'' or more broadly concepts that enhance trades between subcultures (or scientific disciplines in general), and applies the model to the exchanges between the theoretical subcultures of \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
. Section \ref{section:application} reveals and interprets the results of these analyses. Finally, Section \ref{section:discussion} explores the consequences of this work, both for our case study (supersymmetry within \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
) and for the more general question of the plurality of science from a quantitative perspective.
}\DIFdelend \DIFaddbegin \section{\DIFadd{Methods}}\label{section:method}
\DIFaddend 

\DIFdelbegin \section{\DIFdel{Data}%DIFDELCMD < \label{section:data}%%%
}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Data}\label{section:data}}
\DIFaddend 
...
The data were retrieved using the Inspire HEP database \citep{InspireAPI}. Inspire HEP is a platform dedicated to the HEP community and is maintained by members of CERN, DESY, Fermilab and SLAC. It aggregates publications from the \gls{hep} literature, and maintains a list of institutions and collaborations involved in the community, while also publishing job offers. It replaced \DIFdelbegin \DIFdel{SPIRES }\DIFdelend \DIFaddbegin \DIFadd{Spires }\DIFaddend in 2012\footnote{``Physicists, start your searches: INSPIRE database now online'', \textit{Symmetry}, May 24th 2012, \url{https://www.symmetrymagazine.org/breaking/2012/05/24/physicists-start-your-searches-inspire-database-now-online}
}.
...
The database includes data about the contents of the literature (title, summary, sometimes keywords), the authors (name, unique identifier, institutional affiliations), dates corresponding to different events related to each paper, associated experiments, references of the articles. The only data pertaining to the contents of the articles that are consistently available are titles and abstracts. \DIFdelbegin \DIFdel{Most articles }\DIFdelend \DIFaddbegin \DIFadd{Articles }\DIFaddend are categorized according to a classification scheme \DIFdelbegin \DIFdel{mostly built upon arXiv 's}\DIFdelend \DIFaddbegin \DIFadd{compatible with that of the arXiv pre-print platform}\DIFaddend . This scheme includes categories such as \texttt{Theory-HEP}, \texttt{Experiment-HEP}, \texttt{Phenomenology-HEP}, \texttt{Astrophysics}, etc. \DIFdelbegin \DIFdel{, and it relies on arXiv's categories for articles that are available on that platform , adding manual curation when they are not. }\DIFdelend \DIFaddbegin \DIFadd{Categories of papers published on arXiv.org are extracted directly from the platform (where they are defined by the authors, while being subject  to moderation and controls). Categories of papers not published on arXiv.org are assigned manually by curators. Categories of papers inherited from the ancestor of Inspire HEP (Spires) and absent from arXiv.org were derived according to a mapping between Spires' classification and the current arXiv-based classification. In this paper, we rely mostly on three categories which entail most of the high-energy physics literature: }\texttt{\DIFadd{Theory-HEP}}\DIFadd{, }\texttt{\DIFadd{Phenomenology-HEP}}\DIFadd{, but also }\texttt{\DIFadd{Experiment-HEP}}\DIFadd{, which typically entails papers that report empirical results such as statistical analyses of experimental data. }\DIFaddend A portion of the articles between the years 1990 to 1995 were not categorized, which led to some issues with the data collection process, as described in Appendix \ref{appendix:collection}. For this reason, our longitudinal analyses will focus on later years, which does not prevent us from addressing our research questions. \DIFaddbegin \DIFadd{The analysis of subcultures spans over years 1980 to 2020. The years prior to 1980 could also have been interesting for this analysis as well, but the corresponding data was of lower quality.
}\DIFaddend 


\DIFdelbegin \section{\DIFdel{Method}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{section:method}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Social and semantic analysis of subcultures of high-energy physics}\label{section:method_subcultures}
...
This model is then trained on \DIFdelbegin \DIFdel{$N=50,000$ }\DIFdelend \DIFaddbegin \DIFadd{$N=100,000$ }\DIFaddend articles of our database \DIFaddbegin \DIFadd{from 1980 to 2020 }\DIFaddend that belong to any of the following categories: \texttt{Experiment-HEP}, \texttt{Phenomenology-HEP}, and \texttt{Theory-HEP}, \footnote{The fit is performed with the scikit-learn python library \citep{scikit-learn} using L2 regularization.}. The vocabulary used in the regression is the $V$ expressions (n-grams\DIFaddbegin \DIFadd{, up to four word long}\DIFaddend ) among those that belong to predefined syntactic patterns\DIFdelbegin \DIFdel{(up to three word long)}\DIFdelend \DIFaddbegin \footnote{\DIFadd{We choose a subset of the syntactic patterns used to analyze the ACL anthology corpus in \mbox{%DIFAUXCMD
\citealt{omodei_tel-01097702}}\hspace{0pt}%DIFAUXCMD
.}}\DIFaddend , that have the highest ``unithood'' as measured in \citealt{omodei_tel-01097702}\footnote{The ``unithood'' measures ``the degree of strength or stability of syntagmatic combinations or collocations'' \citep{Kageura1996}}. The size of the vocabulary $V$ is chosen to be a round number that is just high enough to reach about the maximum accuracy of the model, as evaluated on the test set (which consists in 10,000 articles not present in the training set). Then, the accuracy of the predictions of the model \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend evaluated using the same test set. The coefficients $\beta_{ci}$ are then analyzed in order to extract the words that are the most discriminatory between ``theory'' and ``phenomenology'', thus revealing the most salient differences. For that, we retrieve those expressions $i$ that maximize $\beta_{\text{th},i}-\beta_{\text{ph},i}$ and $\beta_{\text{ph},i}-\beta_{\text{th},i}$. Because of the inverse document frequency transformation applied prior to the regression, expressions that are more common are favored by this selection process.

\subsubsection{The social divide between Theory and Phenomenology}

What does it mean to say that theory and phenomenology have a ``demographic component'', as  \citet[p.~138]{galison1987how} puts it, regarding theory and experiment in \gls{hep}? It means that these categories of the literature are supplied by distinct groups of people, ``theorists'' and ``phenomenologists''. Therefore, we will investigate whether it is the case that experimental, phenomenological and theoretical papers are published by three distinct groups of physicists, such that these physicists usually contribute mostly to just one of these categories \DIFdelbegin \DIFdel{. We include }\DIFdelend \DIFaddbegin \DIFadd{Again, }\DIFaddend ``experiment''\DIFdelbegin \DIFdel{in our analysis, since it }\DIFdelend \DIFaddbegin \DIFadd{, which is a paradigmatic example of subculture in Galison's view, is also included in our analysis. It }\DIFaddend will be useful to assess whether the distinction between phenomenology and theory is \DIFdelbegin \DIFdel{as strong as }\DIFdelend \DIFaddbegin \DIFadd{comparable to }\DIFaddend the distinction between theory and experiment (the one initially stressed by Galison).
...
\DIFdelbegin \DIFdel{Indeed, }\DIFdelend \DIFaddbegin \DIFadd{The binomial process assumes that each physicist can be imputed a constant latent fraction of papers in each category. The beta prior is a flexible distribution over probabilities, which can be either unimodal or bimodal. The exponential prior over $\alpha$ and $\beta$ is agnostic regarding these two possibilities, and its exact shape does not significantly matter considering the amount of available data. Most crucially for us, }\DIFaddend this model allows to combine information from researchers with many papers and researchers with very \DIFaddbegin \DIFadd{few }\DIFaddend papers; for those with few papers, the estimation of the latent probabilities is more influenced by the shape of the \DIFdelbegin \DIFdel{Beta }\DIFdelend \DIFaddbegin \DIFadd{beta }\DIFaddend distribution. The model was fitted to 2500 researchers randomly sampled among those with more than 3 publications in HEP for 1980-2020. In order to evaluate the social entrenchment of these categories, we verify that most physicists contribute mostly to just one of these categories.
...
Through the covariance matrix $\Sigma$, the CTM model is \DIFdelbegin \DIFdel{therefore }\DIFdelend able to learn \DIFdelbegin \DIFdel{correlation }\DIFdelend \DIFaddbegin \DIFadd{correlations }\DIFaddend between topics, and therefore to account for the fact that some topics are more likely to occur together within one document. Moreover, our intuition is that using CTM allows the derivation of more realistic topic-distribution for short texts such as abstracts, for which the small amounts of words only moderately inform the prior topic distribution. Most importantly, this model allows us to directly assess the level of independence between the topics derived by the model, which is important for assessing the autonomy of the contexts in which supersymmetry arises.

The model is trained on $N=120,000$ articles randomly sampled from those between 1980-2020 that belong to any of the categories \texttt{Theory-HEP}, \texttt{Phenomenology-HEP}, \texttt{Experiment-HEP}, and \DIFaddbegin \DIFadd{also }\DIFaddend \texttt{Lattice} \DIFdelbegin \DIFdel{. The vocabulary selection procedure and the procedure }\DIFdelend \DIFaddbegin \DIFadd{(a theoretical approach to HEP, with ties to both theory and phenomenology, and in which we expected supersymmetry to potentially arise as well). The procedures for extracting the input vocabulary and }\DIFaddend for choosing the hyper-parameters are described in detail in appendices \ref{appendix:data_selection} and \ref{appendix:hyper_parameter} respectively. Two methodological contributions can be highlighted. First, we included informative n-grams matching pre-defined syntactic patterns in the vocabulary in order to preserve more semantic information. Second, we made a prudent and balanced use of perplexity and topic coherence measures in order to recognize the advantages and limitations of both these kinds of measures for assessing the quality of topic models and choosing the best hyper-parameters. \DIFaddbegin \DIFadd{The procedure resulted in the extraction of 75 topics.
}\DIFaddend 
...
In section \ref{section:application_plurality}, the model is applied to a number of tasks: the evaluation \DIFaddbegin \DIFadd{of }\DIFaddend the contexts (i.e. topics) in which supersymmetry occurs in the literature, the extent of the correlation between these contexts and finally the trends in research involving supersymmetry since the start of the \gls{lhc}.
...
Model accuracy & 91\%   & \DIFdelbeginFL \DIFdelFL{85}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{86}\DIFaddendFL \%          & 92\%       &  \\
...
Our quantitative analysis supports our claim that\DIFaddbegin \DIFadd{, at least over the years 1980 to 2020, }\DIFaddend theory and phenomenology should be regarded as distinct subcultures with partially distinct languages. Consequently, strategies ought to be devised for them to properly communicate and coordinate their efforts\DIFaddbegin \DIFadd{, as long as physicists believe it to be necessary or worthwhile}\DIFaddend . It follows that their unity should not be assumed; instead, why a trading zone may be successfully worked out remains to be explained. Before we turn to the ability of supersymmetry to sustain the coordination between these subcultures, we will address the plurality of supersymmetry research itself.
...
Where $D_y$ is the amount of articles first submitted in year $y$. We then selected the three topics with the highest increase (\DIFdelbegin \DIFdel{hot }\DIFdelend \DIFaddbegin \DIFadd{rising }\DIFaddend topics) and decrease (\DIFdelbegin \DIFdel{cold }\DIFdelend \DIFaddbegin \DIFadd{declining }\DIFaddend topics) in magnitude over this period. For that, $P(z|y)$ was fitted to a linear time trend ($P(z|y) = a_zy+b_z$), discarding topics for which the correlation was not significant (i.e. $R=0$ is excluded from the 99\% CI). Then, the topics were sorted according to the best fit value of $a_z$, the rate of increase of its magnitude per year (similarly to what was done \DIFdelbegin \DIFdel{by }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend \citealt{Griffiths2004}). We apply the procedure to  all papers mentioning at least one of the words ``supersymmetric'', ``supersymmetry'' or ``susy'' in their title or abstract in the years following the start of the \gls{lhc}. The results are shown in Figure  \ref{fig:hot_cold_topics_hep_2011_2019_susy}. 


\begin{figure*}
    \centering
    \includegraphics{Fig5.eps}
    \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Cold and hot topics among those that mention supersymmetry since the first results of the LHC (2011-2019).}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Declining and rising topics among those that mention supersymmetry since the first results of the LHC (2011-2019).}} \DIFaddendFL On the left, the three \DIFdelbeginFL \DIFdelFL{coldest }\DIFdelendFL topics \DIFaddbeginFL \DIFaddFL{that }\DIFaddendFL are \DIFaddbeginFL \DIFaddFL{declining the fastest }\DIFaddendFL ``Supersymmetric particles'', ``Higgs sector beyond the SM'' and ``Higgs boson''. On the right, the three \DIFdelbeginFL \DIFdelFL{hottest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fastest rising }\DIFaddendFL topics are ``Supergravity'', ``Amplitudes and Feynman Diagrams'', ``Conformal Field Theory''.}
...
In order to understand these dynamics, it is therefore \DIFdelbegin \DIFdel{useful }\DIFdelend \DIFaddbegin \DIFadd{necessary }\DIFaddend to distinguish theoretical supersymmetry from phenomenological supersymmetry. As physicist Mikhail Shifman argued in an early assessment of the first results of the \gls{lhc} in 2012, 
...
Shifman's assessment strikingly converges with the patterns that emerge from our analysis. Topic models reveal the plurality of supersymmetry in high-energy physics. They support that supersymmetry arises in different contexts, some theoretical and others phenomenological. They allowed us to demonstrate that these ``faces'' of supersymmetry have responded differently to the absence of evidence for supersymmetric particles at the \gls{lhc}. \DIFaddbegin \DIFadd{Indeed, although phenomenologists find supersymmetry to be much less valuable in the light of the most recent experimental findings, theorists may still rely on it for their own endeavor. }\DIFaddend This supports that cultures can ``trade'' certain concepts (according to Galison's terminology) while retaining much of their autonomy\DIFaddbegin \DIFadd{, including in their own appraisal of the usefulness of these concepts}\DIFaddend \footnote{``trading partners can hammer out a local coordination, despite vast global differences.'' \citep[p.~783]{galison1997image}}. 
...
\DIFdelbegin \DIFdel{Where do trading zones occur }\DIFdelend \DIFaddbegin \DIFadd{Which concepts sustain trades }\DIFaddend within \gls{hep}? As \DIFdelbegin \DIFdel{discussed in the Method section, citations can signal trades between subcultures. We apply the methodology }\DIFdelend proposed in Section \ref{section:method_trading_zone}\DIFdelbegin \DIFdel{for localizing trading zones. We }\DIFdelend \DIFaddbegin \DIFadd{, we }\DIFaddend measure the ability of certain concepts (keywords) to sustain trades through time \DIFdelbegin \DIFdel{-- most specially, exports from the theoretical subculture to the phenomenological subculture -- }\DIFdelend in terms of the probability that each of these concepts occurs in citations across \DIFdelbegin \DIFdel{these two categories of the literature}\DIFdelend \DIFaddbegin \DIFadd{theory and phenomenology}\DIFaddend . The results are shown in Figures \ref{fig:cross_citations_probs_th_ph} and \ref{fig:cross_citations_probs_ph_th}.
...
% \end{quote}
Eventually, the \gls{lhc} provided ``a test of the unity of physics''\footnote{ \citet[p.~29]{Wilson1986} (cited in \citealt[p.~292]{Cat1998}) used this expression in reference to the now aborted Super-Conducting Supercollider.}, and its verdict was ruthless. In the future, will \DIFdelbegin \DIFdel{a shared belief in unity be maintained}\DIFdelend \DIFaddbegin \DIFadd{the field strive to regain unity (possibly to the detriment of certain research programs)}\DIFaddend , or will the socially entrenched divergences between these ``cultures'' of high-energy physics prevail?  We may assume that the challenge is merely transitory, and that theorists will eventually move to other theories which will be more successful from an empirical or phenomenological standpoint. However, the divergence between these theoretical cultures has become axiological \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Ritson2021,Laudan1984}}\hspace{0pt}%DIFAUXCMD
, and it }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Camilleri2015,Laudan1984}}\hspace{0pt}%DIFAUXCMD
, in the sense that they prioritize different epistemic goals}\footnote{ \DIFadd{\mbox{%DIFAUXCMD
\citet{Laudan1984} }\hspace{0pt}%DIFAUXCMD
refers to disagreements in the goals of scientific inquiry as }\textit{\DIFadd{axiological}} \DIFadd{disagreements. \mbox{%DIFAUXCMD
\citealt{Camilleri2015}}\hspace{0pt}%DIFAUXCMD
, for instance, have argued that certain controversies around string theory could be understood in terms as an instance of axiological disagreement.}}\DIFadd{; and this divergence }\DIFaddend may persist as long as their differences in aims persist; as \citeauthor{galison1997image} puts it, ``there is no teleological drive towards ever-greater cohesion'', and ``fields previously bound [may] fall apart'' \citep[p.~805]{galison1997image}. As illustrated in Figure \ref{fig:mssm}, the aims of the theorists is to achieve the unification of the fundamental forces and a coherent theory of quantum gravity. By contrast, the aim of phenomenologists is to guide the experiment towards promising directions where evidence of ``new physics'' may be found. Both these aims may seem well-founded; however, there is no reason to expect that a simultaneous solution can be worked out. The apparent failure of supersymmetry to provide such a simultaneous solution does not undermine by itself the relevance of the ``theorists''' aims, nor does it undermine the methodology they deploy for addressing their goals (e.g. their trust in certain theoretical constraints, cf. \citealt{Galison1995}). It does, however, challenge the belief that such methods can provide grounds for progress to the field \textit{as a whole}; indeed, unification and quantum gravity might eventually not provide much reliable guidance to the experimental side\DIFdelbegin \DIFdel{; and conversely}\DIFdelend \DIFaddbegin \DIFadd{. Conversely}\DIFaddend , it can very well be that the details of the theory ``at high energy'', where quantum effects matter to gravity, cannot be extrapolated from our knowledge of the low-energy theory, i.e. the one that we can probe in our experiments\DIFdelbegin \footnote{\DIFdel{More drastically, \mbox{%DIFAUXCMD
\citet{Cao1993} }\hspace{0pt}%DIFAUXCMD
expressed the view that the theories at different energy scales (i.e. corresponding to different ranges of phenomena) are irreducible, and he argued for a ``pluralist view of possible theoretical ontologies'' while challenging the possibility of achieving a ``ultimate stable theory  of everything'' (p.~69--71). According to this view, the plurality of ontologies in physics is not an accident but the result of partially disconnected ``phenomenological domains'' through which knowledge cannot be deduced from one another. For a criticism of this view, see \mbox{%DIFAUXCMD
\citealt{Rivat2020}}\hspace{0pt}%DIFAUXCMD
.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{. As a result, \mbox{%DIFAUXCMD
\citet{Dawid2013} }\hspace{0pt}%DIFAUXCMD
argues for recourse to meta-empirical assessment of theories in theoretical physics, given that empirical input under-determines the directions of potential progress in quantum gravity. }\DIFaddend Disagreements in the aims of a scientific enterprise may not always be resolved on purely epistemic ground, and a resolution, provided it occurs, may involve some sort of negotiation instead.  As long as theorists believe in the feasibility of their aims, they may pursue these aims even if it further isolates them from other cultures\DIFaddbegin \footnote{\DIFadd{More drastically, \mbox{%DIFAUXCMD
\citet{Cao1993} }\hspace{0pt}%DIFAUXCMD
expressed the view that the theories at different energy scales (i.e. corresponding to different ranges of phenomena) are irreducible, and they argued for a ``pluralist view of possible theoretical ontologies'' while challenging the possibility of achieving a ``ultimate stable theory  of everything'' (p.~69--71). According to this view, the plurality of ontologies in physics is not an accident but the result of partially disconnected ``phenomenological domains'' through which knowledge cannot be deduced from one another. For a criticism of this view, see \mbox{%DIFAUXCMD
\citealt{Rivat2020}}\hspace{0pt}%DIFAUXCMD
.}}\DIFaddend . Alternatively, they could decide that the schism should be resolved; as Galison puts it, distinct scientific cultures ``can [\dots] understand that the continuation of exchange is a prerequisite to the survival of the larger culture of which they are part'' \citep[p.~803]{galison1997image}.%In any case, it is reasonable to expect that non-epistemic factors may influence such ``decisions''.
...
\DIFaddbegin \DIFadd{Finally, the historical scope of our analysis was limited by our database. In particular, we were only able to analyze the theory/phenomenology divide over a restricted time range (1980--2020), and we could not reveal how such a divide has historically emerged. By contrast, Galison has proposed a number of explanations for the earlier decoupling of theory and experiment, such as increased specialization and the increased time-scales of experiments \mbox{%DIFAUXCMD
\citep[p.~138]{galison1987how}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFaddend \printglossary[type=\acronymtype,title=List of abbreviations]
...
\DIFadd{The authors acknowledge support from the Open Access Publication Fund of the University of Wuppertal.
}

\DIFaddend \section*{Conflicts of interest}
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  \bibliographystyle{spbasic}
\DIFaddbegin \printbibliography
\DIFaddend 
...
\DIFaddbegin \subsection{\label{appendix:stability}\DIFadd{Text-classifier performance stability}}

\DIFadd{The categories (}\texttt{\DIFadd{Theory-HEP}}\DIFadd{, }\texttt{\DIFadd{Phenomenology-HEP}} \DIFadd{and }\texttt{\DIFadd{Experiment-HEP}}\DIFadd{) that we trained our classifier (\ref{section:method_subcultures}) to predict have been assigned in different ways in the Inspire HEP database. Although a majority were categorized based on arXiv's classification system, some papers were not, especially those published before arXix was introduced (in the early 1990s). It might seem unclear whether these classification procedures are consistent and revealing of distinct underlying cultures. In order to demonstrate that it is the case, in Figure \ref{fig:stability}, we show that the performance of the text-classifier is nonetheless roughly stable throughout the period considered (1980--2020). To this end, we subdivide this time-range in bins of five years and perform k-fold cross-validation using each five-year bin as a validation set. Accuracy remains high and approximately stable; therefore, these various classification procedures, and the underlying identity of each of these subcultures, must be rather consistent over this period.
}

\begin{figure}
    \centering
    \includegraphics{Fig8.eps}
    \caption{\DIFaddFL{Accuracy of the text-classifier from Section \ref{section:method_subcultures} as a function of the papers' years of publication. Error-bars represent the 95\% confidence interval. Dashed lines show the accuracy of the baseline model (which may vary only due to variations in the frequency of each category, since the baseline model always predicts the most common class). The accuracy is roughly constant across time for each of the three categories, despite significant variations in the frequency of each class.}}
    \label{fig:stability}
\end{figure}



\DIFaddend \subsection{Topic model}
...
\DIFdelbegin \DIFdel{Now equipped with these results, we can turn to applications..
}\DIFdelend 
...
\DIFdelbegin \subsection{%DIFDELCMD < \label{appendix:phenomenology_centrality}%%%
\DIFdel{The centrality of phenomenology in bonding \mbox{%DIFAUXCMD
\gls{hep}}\hspace{0pt}%DIFAUXCMD
}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\label{appendix:topics_categories}\DIFadd{Topics and their correlation with categories}}
\DIFaddend 

\DIFaddbegin \DIFadd{Below, we evaluate how topics compare with the classification of the literature. For that, we generated a 2D representation of the semantic space by applying a t-SNE transformation \mbox{%DIFAUXCMD
\citep{Maaten2008} }\hspace{0pt}%DIFAUXCMD
on the distance matrix $1-R_{ij}$, where $R_{ij}$ is the correlation matrix for the 75 topics from the CTM. The t-SNE transformation aims to reduce dimensionality (from 75 to 2) while preserving distances, such that highly correlated topics should appear close to each other on the resulting 2D map. We then colored each topic according to the category (among theory, phenomenology and experiment) that has the strongest association (normalized pointwise mutual information) with this topic. The graph was then rotated such that the x-axis would explain most of the variance in these three categories. Topics related to supersymmetry were emphasized and labeled. The resulting map is shown in Figure \ref{fig:tsne}.
}

\begin{figure}
\centering
\includegraphics{Fig9.eps}
\caption{\label{fig:tsne}\DIFaddFL{Semantic map extracted from the topic model, after applying a t-SNE transformation. Each dot represents a topic. Each topic is assigned the category, among theory, phenomenology and experiment, that is most associated with it. Correlated topics appear closer to each other. For each category, the density of topics along the x-axis is shown in the lower plot.}}
\end{figure}

\DIFadd{Although the t-SNE transformation does not yield very stable results, it generally appears (as in this figure) that topics most associated with a given category (e.g. theory) appear closer to each other, such that these three categories explain part of the variance in the semantic space. Second, in this representation, the distinction between phenomenological supersymmetry and theoretical supersymmetry is supported by the emergence of two separate clusters of supersymmetry-related topics.
}


\subsection{\label{appendix:phenomenology_centrality}\DIFadd{Validity of the citation network for exploring the trading zone}}

\DIFadd{Below, we support the relevance of the citation network as a means of exploring trading zones between scientific cultures by showing it can be used to recover known facts, in particular i) that theory and experiment in HEP do not communicate directly and ii) that phenomenology channels most exchanges across them.
}

\DIFaddend We build a citation network where each node is one paper of the literature and the edge between nodes $x$ and $y$ is assigned a weight $w_{x,y}=1$ if $x$ cites $y$ and 0 otherwise. From this we can define the amount of citations of papers from the category $i$ to a paper from the category $j$ as: 
